...
if (isset($_POST['update'])){
    if (do_edit('location', $_POST, $_GET['id'])){
        global $edited;
        $edited=1;
        MAD_Admin_Redirect::redirect('edit_location.php?edited=1&id='.$_GET['id']);	
    }
    else
    {
        global $edited;
...
if($edited!=2){
    $editdata=get_location_details($_GET['id']);
    $editdata['location_name'] = $editdata['adv_location_name'];
    $editdata['location_lat'] = $editdata['adv_location_lat'];
    $editdata['location_lon'] = $editdata['adv_location_lon'];
}
           ...
           <?php if (isset($errormessage)){ ?>
            ...
            <?php } ?>
				...
				?>	
                     ...
                     <div class="actions">						
									<button type="submit" class="btn btn-quaternary btn-large">Edit Location</button>
								</div> <!-- .actions -->
