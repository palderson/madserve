...
require dirname(realpath(__FILE__)) . '/../../libs/GeoLocation/AnthonyMartin/GeoLocation/GeoLocation.php';
...
function get_locations(){
    ...
    $usrres=mysql_query("select * from md_ad_unit_locations", $maindb);

    while($location_detail=mysql_fetch_array($usrres)){
            echo '<tr class="gradeA">
            <td>'.$location_detail['adv_location_name'].'</td>
            <td class="center">'.$location_detail['adv_location_lat'].' </td>
            <td class="center">'.$location_detail['adv_location_lon'].' </td>';
        echo '<td class="center"><span class="ticket ticket-info"><a href="edit_location.php?id='.$location_detail['adv_location_id'].'" style="color:#FFF; text-decoration:none;">Edit Location</a></span>&nbsp;&nbsp;<span class="ticket ticket-important"><a style="color:#FFF; text-decoration:none;" href="#" data-location-id="'.$location_detail['adv_location_id'].'">Delete</a></span></td>';
        echo '</tr>	';
    }
}

function get_ad_locations($adId){
	global $maindb;	
    
    $usrres=mysql_query("select loc.adv_location_id, loc.adv_location_name, loc.adv_location_lat, loc.adv_location_lon, map.adv_location_radius from md_ad_unit_locations loc LEFT JOIN md_ad_unit_to_locations map ON (loc.adv_location_id=map.adv_location_id) WHERE map.adv_id=$adId", $maindb);
	
	$location_details = array();

    while($location_detail=mysql_fetch_assoc($usrres)){
...
function get_locations_details(){
    global $maindb;
    $getloctyperes=mysql_query("select * from md_ad_unit_locations", $maindb);
    $location_details = array();
    while( $location_detail=mysql_fetch_assoc($getloctyperes) ){
        $location_details[] = $location_detail;
    }
    return $location_details;
}
...
function get_location_details($id){
    ...
    $getloctyperes=mysql_query("select * from md_ad_unit_locations WHERE adv_location_id='$id'", $maindb);

    return mysql_fetch_assoc($getloctyperes);
}
...
function do_edit($type, $data, $detail){
    ...
    if ($type=='adunit'){
		...
		// modifying the md_ad_unit_to_locations table involves 3 steps
		// Updating  -  array_intersection()   - already selected
		// Inserting -  array_diff($new, $old) - newly selected
		// Deleting  -  array_diff($old, $new) - unchecked old
		
		
		$alreadySelectedLocations = get_ad_locations($detail);
		
		$selectedLocationIds = array();
		
		foreach($alreadySelectedLocations as $location){
			$selectedLocationIds[] = $location['adv_location_id'];
		}
		
		$toBeUpdated  = array_intersect($data['selected_locations'], $selectedLocationIds);
		$toBeInserted = array_diff($data['selected_locations'], $selectedLocationIds);
		$toBeDeleted  = array_diff($selectedLocationIds, $data['selected_locations']);
		

                /*
		foreach($toBeUpdated as $locationId){
			mysql_query("UPDATE md_ad_unit_to_locations SET adv_location_radius='{$data["location_radius_$locationId"]}'WHERE adv_id=$detail AND adv_location_id=$locationId", $maindb);
		}
		
		foreach($toBeInserted as $locationId){
			mysql_query("INSERT INTO md_ad_unit_to_locations (adv_id, adv_location_id, adv_location_radius) VALUES ($detail, $locationId, '{$data["location_radius_$locationId"]}')", $maindb);
		}
		
		foreach($toBeDeleted as $locationId){
			mysql_query("DELETE FROM md_ad_unit_to_locations WHERE adv_id=$detail AND adv_location_id=$locationId", $maindb);
		}
                 */

                error_reporting(E_ALL);
		foreach($toBeUpdated as $location_id){
                    ...
                    mysql_query("UPDATE md_ad_unit_to_locations SET adv_location_radius=$radius, adv_location_bound_lat_min=$latMin, adv_location_bound_lon_min=$lonMin, adv_location_bound_lat_max=$latMax, adv_location_bound_lon_max=$lonMax WHERE adv_id=$detail AND adv_location_id=$location_id", $maindb);

		}
		
		foreach($toBeInserted as $location_id){
                    ...
                    mysql_query("INSERT INTO md_ad_unit_to_locations (adv_id, adv_location_id, adv_location_radius, adv_location_bound_lat_min, adv_location_bound_lon_min, adv_location_bound_lat_max, adv_location_bound_lon_max) VALUES ('$detail', '$location_id', '$radius', '$latMin', '$lonMin', '$latMax', '$lonMax')", $maindb);
    ...
    if ($type=='location'){
        if (empty($data['location_name'])){
            ...
            $editdata=$data;
            return false;	
        } else if(!is_numeric($data['location_lat'])){
            global $errormessage;
            $errormessage='Invalid latitude';
            global $editdata;
            $editdata=$data;
            return false;	
        } else if(!is_numeric($data['location_lon'])){
            global $errormessage;
        ...
        $data['location_name']=sanitize($data['location_name']);

        mysql_query("UPDATE md_ad_unit_locations set adv_location_name='$data[location_name]', adv_location_lat='$data[location_lat]', adv_location_lon='$data[location_lon]' where adv_location_id='$detail'", $maindb);
...
function delete_campaign($id){
    ...
    $adUnitsQuery = mysql_query("SELECT * from md_ad_units where campaign_id='$id'", $maindb);
    $adUnitIds = array();

    while($adUnit = mysql_fetch_assoc($adUnitsQuery)){
        $adUnitIds[] = $adUnit['adv_id'];
    }

    delete_adunits($adUnitIds);
...
function delete_adunits($ids){
    global $maindb;
    foreach($ids as $id){
        mysql_query("DELETE from md_ad_units where adv_id ='$id'", $maindb);
        mysql_query("DELETE from md_ad_unit_to_locations where adv_id ='$id'", $maindb);
    }
}
...
function delete_creativeserver($id){
    ...
    $adUnitsQuery = mysql_query("SELECT * from md_ad_units where adv_type='1' AND creativeserver_id='$id'", $maindb);
    $adUnitIds = array();
